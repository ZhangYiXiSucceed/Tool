...
void Database::InitUI()
{
    ...
    ui->customPlot->graph(2)->setBrush(QColor(0, 0, 255, 20));
    ...
    for (int i=0; i<dataCount; ++i)
    {
      ...
      dataMinusSinExp[i].value = -qSin(dataMinusSinExp[i].key)*qExp(
    dataMinusSinExp[i].key);
...
{
    connect(ui->tbnFrequencyDomainLast,SIGNAL(clicked(bool)),this,SLOT(DisplayFrequencyDomainLast()));
}
void Database::DisplayFrequencyDomainLast()
{
    emit Display(1);
}
